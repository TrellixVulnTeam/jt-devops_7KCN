#!/bin/bash
BASE_DIR=/data/jtb/infra/git/jt-platform-ytsf
REPO_URL="http://git.zanclick.cn/jtb/jtb-ytsf/jt-platform-ytsf.git"
BRANCH=1.1.x
NEW_VERSION=$1
REL_VERSION=$2
...
depend_svc=platform-provider-report-ytsf
replace_version () {
  OLD_VERSION=`grep "</version>" ${BASE_DIR}/${depend_svc}/pom.xml |awk -F '[<>]' 'NR==1{print $3}'`
  Result=`find ${BASE_DIR} -name "pom.xml"`
  for dir in ${Result};do
    sed -i "s;${OLD_VERSION};$1;"  $dir
  done
  retval=`grep -w "$1"  ${BASE_DIR}/${depend_svc}/pom.xml`
    if [ -z ${retval} ];then
